...
import { getBrandTokens, TOKKOBROKER } from './brands'

export { getBrandTokens, TOKKOBROKER }

/**
 * Build the theme for a brand.
 *
 * @param {Object} [brand] Brand tokens: `{ name, colors, fontFamily }`.
 *   Defaults to Tokko Broker.
 * @returns {Object} Theme consumable by styled-components.
 */
export const createTheme = (brand) => {
  const { name, colors, fontFamily } = getBrandTokens(brand)
...
/**
 * Read the brand theme from the nearest JopiThemeProvider.
 *
 * Components must keep working when rendered outside a provider, so this falls
 * back to the Tokko Broker theme instead of returning undefined.
 */
export const useBrandTheme = () => {
  const contextTheme = useTheme()

  return contextTheme && contextTheme.colors ? contextTheme : defaultTheme
}
