...
// A clearly distinguishable "edited" result so the sync is visually obvious:
// the preview switches to this green tile once the edit is applied.
...
// Story to verify the Original/Editada toggle behavior together with the edit
// sync. Only the second image starts as "edited", with its `editedSrc` equal to
// its own `src`, so toggling shows no visual change yet. Clicking that image's
// edit action swaps its `editedSrc` for the simulated edited tile, making the
// Original/Editada difference obvious.
export const EditSync = ({ listOfImages, texts, ...args }) => {
  const [images, setImages] = useState(() =>
    listOfImages.map(({ src }, index) =>
      index === 1
        ? { src, editedSrc: src, isEdited: true }
        : { src, isEdited: false }
    )
  )

  // Simulate finishing an edit: the parent updates `listOfImages` with the
  // edited source for the clicked item, mirroring real app behavior.
  const handleEdit = (item) => {
    setImages(prev =>
      prev.map(image =>
        image.src === item.src
          ? { ...image, editedSrc: SIMULATED_EDITED_SRC, isEdited: true }
          : image
      )
    )
  }

  // Surface what the toggle does: which version each item is showing.
  const handleSwitchChange = (item, isEditedActive) => {
    // eslint-disable-next-line no-console
    console.log('onSwitchChange', { src: item.src, showing: isEditedActive ? 'edited' : 'original' })
  }
